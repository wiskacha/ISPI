    ...
    <div class="block block-rounded" style="margin-top: 1rem; margin-left: 1rem; margin-right: 1rem;">
        ...
        <div class="block-content block-content-full">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form id="editDetalleForm{{ $detalle->id_detalle }}"
                                                        action="{{ route('movimientos.actualizarDetalle', $detalle->id_detalle) }}"
                                                        method="POST">
                                                        @csrf
                                                        <div class="row mb-3">
                                                            <!-- Producto Field -->
                                                        ...
                                                        @if ($movimiento->tipo == 'ENTRADA')
                                                            <input type="hidden"
                                                                id="idProveedor{{ $detalle->id_detalle }}"
                                                                value="{{ $movimiento->id_proveedor }}">
                                                        @endif
                                                    </form>
                                                </div>
            ...
            <button class="btn btn-secondary"
                onclick="window.location.href='{{ route('movimientos.edit', $movimiento->id_movimiento) }}'"> ←
                Regresar</button>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addDetalleForm"
                        action="{{ route('movimientos.guardarDetalle', $movimiento->id_movimiento) }}" method="POST">
                        @csrf
                        <div class="row mb-3">
                            <!-- Producto Field -->
                            <div class="col-sm-6 col-md-4 col-lg-3">
                        ...
                        @if ($movimiento->tipo == 'ENTRADA')
                            <input type="hidden" id="idProveedor" value="{{ $movimiento->id_proveedor }}">
                        @endif
                    </form>
                </div>
                <div class="modal-footer">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('submit', '#addDetalleForm', function(event) {
                ...
                console.log(tipoMovimiento, idAlmacen, idProveedor, productoNombre,
                    cantidadSolicitada); // Agregar esta línea para depurar
                // Establecer la URL según el tipo de movimiento
                let url;
                if (tipoMovimiento === 'SALIDA') {
                    url = '{{ route('movimientos.checkAvailability') }}';
                } else if (tipoMovimiento === 'ENTRADA') {
                    url = '{{ route('movimientos.checkContacto') }}';
                }
            ...
            $(document).on('submit', 'form[id^="editDetalleForm"]', function(event) {
                ...
                const idProveedor = $(`#idProveedor${detalleId}`).val();
                const csrfToken = '{{ csrf_token() }}';
